...
class OAuthController extends Controller
{
    ...
    public function __construct(
        ...
        $this->userPanel = FilamentOrganizations::getUserPanel();
    }
    ...
    {
        session()->put('filament-organizations.previous_url', url()->previous());
    ...
    public function handleProviderCallback(Request $request, string $provider, ResolvesSocialiteUsers $resolver): RedirectResponse | LoginResponse
    {
        ...
        $account = FilamentOrganizations::findConnectedAccountForProviderAndId($provider, $providerAccount->getId());
    ...
    {
        $previousUrl = session('filament-organizations.previous_url');
    ...
    protected function handleError(Request $request): RedirectResponse
    {
        ...
        if ($error_description === null) {
            $error_description = __('filament-organizations::default.errors.generic_error');
        }
    ...
    {
        $user = FilamentOrganizations::newUserModel()->where('email', $providerAccount->getEmail())->first();
    ...
    protected function handleSignInNotFound(string $provider): RedirectResponse
    {
        return $this->redirectToWithError(
            __('filament-organizations::default.errors.signin_not_found', compact('provider')),
            $this->loginUrl,
        );
    }
    ...
    {
        if (FilamentOrganizations::newUserModel()->where('email', $providerAccount->getEmail())->exists()) {
            return $this->redirectToWithError(__('filament-organizations::default.errors.already_connected', compact('provider')), $this->loginUrl);
        }
    ...
    {
        $title = __("filament-organizations::default.notifications.{$translationKey}.title");
        $body = __("filament-organizations::default.notifications.{$translationKey}.body", $translationParameters);
    ...
    protected function handleUserAlreadyRegistered(Authenticatable $user, ?ConnectedAccount $account, string $provider, ProviderUser $providerAccount): RedirectResponse | LoginResponse
    {
        ...
        return $this->redirectToWithError(
            __('filament-organizations::default.errors.already_associated_account', compact('provider')),
    ...
    protected function register(string $provider, ProviderUser $providerAccount): RedirectResponse | LoginResponse
    {
        ...
        if ($email === null) {
            return $this->redirectToWithError(__('filament-organizations::default.errors.no_email_with_account', compact('provider')), $this->registrationUrl);
        }

        if (FilamentOrganizations::newUserModel()->where('email', $email)->exists()) {
            return $this->redirectToWithError(__('filament-organizations::default.errors.email_already_associated', compact('provider')), $this->registrationUrl);
        }
    ...
    {
        return redirect($url)->withErrors(['filament-organizations' => $message]);
    }
