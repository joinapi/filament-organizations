<x-filament-organizations::grid-section md="2">
    <x-slot name="title">
        {{ __('filament-organizations::default.grid_section_titles.profile_information') }}
    </x-slot>

    <x-slot name="description">
        {{ __('filament-organizations::default.grid_section_descriptions.profile_information') }}
    </x-slot>

    <x-filament::section>
        <x-filament-panels::form wire:submit="updateProfileInformation">
            <!-- Profile Photo -->
            @if (Joinapi\FilamentOrganizations\FilamentOrganizations::managesProfilePhotos())
                <div x-data="{ photoName: null, photoPreview: null }" class="space-y-2">
                    ...
                    <x-filament-forms::field-wrapper.label for="photo">
                        {{ __('filament-organizations::default.labels.photo') }}
                    ...
                    <x-filament::button size="sm" x-on:click.prevent="$refs.photo.click()">
                        {{ __('filament-organizations::default.buttons.new_photo') }}
                    </x-filament::button>

                    @if ($this->user->profile_photo_path)
                        <x-filament::button size="sm" color="danger" wire:click="deleteProfilePhoto">
                            {{ __('filament-organizations::default.buttons.remove_photo') }}
                        </x-filament::button>
                    @endif

                    <x-filament-organizations::input-error for="photo" />
                </div>
            @endif

            <!-- Name -->
            <x-filament-forms::field-wrapper id="name" statePath="name" required="required" label="{{ __('filament-organizations::default.fields.name') }}">
