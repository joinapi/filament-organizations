...
<x-filament-panels::page>
    ...
    <x-filament::modal id="displayingToken" icon="heroicon-o-key" icon-color="primary" alignment="{{ $modals['alignment'] }}" footer-actions-alignment="{{ $modals['formActionsAlignment'] }}" width="{{ $modals['width'] }}">
        <x-slot name="heading">
            {{ __('filament-organizations::default.modal_titles.token') }}
        </x-slot>

        <x-slot name="description">
            <div>
                {{ __('filament-organizations::default.modal_descriptions.copy_token') }}
            </div>
        </x-slot>

        <x-filament-organizations::input x-ref="plaintextToken" type="text" readonly="readonly" :value="$plainTextToken"
        ...
        @if($modals['cancelButtonAction'])
            <x-slot name="footerActions">
                <x-filament::button color="gray" wire:click="cancelDisplayingToken">
                    {{ __('filament-organizations::default.buttons.close') }}
                </x-filament::button>
            </x-slot>
        @endif
    </x-filament::modal>
